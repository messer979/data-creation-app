...
#transactions
class DMUIFacade(Component):
    def __init__(self, activeUser):
        '''dmui-facade/api/dmui-facade'''
        self.activeUser = activeUser
        self.methods = dict(search_services=search_services, general_services=general_services, get_services=get_services, import_services=import_services)
        super().__init__(activeUser, 'dmui-facade')
    ...
    def search_item_inventory(self, item_view={}):
        endpoint = "/dmui-facade/api/dmui-facade/inventory/itemInventory/search"
        url = self.activeUser.wm_app + endpoint
    ...
    def search_component(self, component_name):
        endpoint = "/dmui-facade/api/dmui-facade/config/attribute/{component_name}"
        url = self.activeUser.wm_app + endpoint
        res = requests.post(url, headers=self.activeUser.headers, data=json.dumps(activity_view_template))
        return response._response_handler(res, verbose=False, view=True)        

    def search_lookup_keys(self, component_name, view_name):
        endpoint = f"/dmui-facade/api/dmui-facade/extension/view/lookupKeys/{component_name}/{view_name}"
        url = self.activeUser.wm_app + endpoint
